...
* 최적화 기법중 Newton’s Method와 Gradient Descent 방법에 대해 알고 있나요?
    * Newton’s Method: Convex한 2차 테일러 근사식에서 최솟값을 찾으며, 원래 식의 최솟값에 근사적으로 접근하는 방법입니다.
        * 근사식에서 그래디언트가 0인 지점을 찾고, 해당 지점을 기준으로 한 근사식에서 0인 지점을 찾는 과정을 반복해서 해에 접근함
...
* Local Minima 문제에도 불구하고 딥러닝이 잘 되는 이유는?
    ...
    * 오히려, Local Minima보다 빈번하게 발생하는 Critical Point인 Saddle Point임
...
* Dropout?
    ...
    * 또한 노드 간에 의존성을 끊어냈기 때문에 하나의 모델 아키텍처에서 상관관계가 적은 다양한 결과를 Aggregation 하는 것으로 해석할 수 있어, 마치 상관관계가 적은 앙상블을 사용하는 것과 같은 효과를 가져와 성능 향상을 기대해볼 수 있습니다.
...
* Batch Normalization?
    ...
    * Standardization는 평균을 뺴주고 표준편차로 나눠주는 작업으로 과적합 문제를 해소하고, 수렴속도를 높일 수 있습니다. 
...
* CNN이 아닌 MLP로 해도 잘 될까?
    * CNN은 Locality와 Parameter Sharing을 통해 동일한 개체가 이미지 내에 위치가 바뀌어도 똑같은 특징을 추출할 수 있기 때문에 복잡한 패턴을 잘 포착할 수 있기 때문에 일반화가 쉽습니다.
        * 여러 층을 통해 계층적으로 특징을 추출하면서 복잡한 패턴을 포착해낼 수 있음
    * 반면에 MLP는 공간적 관계정보를 활용하지 않기 때문에 동일한 개체가 이미지 내에 위치가 바뀌면 다른 특징을 추출하기 때문에 복잡한 패턴을 잘 포착하지 못하기 때문에 일반화가 어렵습니다.
